...
egg_bomb_sound = pygame.mixer.Sound('audio/egg_bomb.mp3')  # Played when the egg bomb clears the screen
...
def handle_enemy_hit(obs, obstacles):
    """Handles a standard enemy collision, reduces lives, sets invincibility, and spawns a splatter.
    Returns True if the obstacle was removed so it stops checking this frame."""
    global lives, is_playing, high_score, invincible_timer
    if current_ticks >= invincible_timer:
        ...
        add_splatter(obs)
        obstacles.remove(obs)  # Clear obstacle if player is already invincible
        return True
    return False
...
def collisions(player, obstacles):
    ...
    global lives, is_playing, high_score, score, stars_collected, bonus_score
    if obstacles:
        for active_obs in obstacles[:]:
            if active_obs['rect'].colliderect(player):
                obs_type = active_obs['type']
                if obs_type == 'star':
                    ...
                    # All other enemies (egg, egg_knight, sunnyside_up, eggsaucer) use the same hit logic
                    if handle_enemy_hit(active_obs, obstacles):
                        break
        return is_playing
    return True
